...
% this is Phase 3 training protocol, added in punishment
% also add in trials without preparation period (readycue) 10%
% add in NoGo trials ... working 9-20
global BpodSystem

%% Define parameters
disp('Lick_LRTG_P_GoNg')
...
%     S.GUI.RespDur = 3; % response window licks get rewarded
    ...
    S.GUI.TimeOutDur = 6;% time out if respond during ready cue
    S.GUI.MinITI = 10;
    S.GUI.MaxITI = 15;
...
TrialTypes (1:2) = [1 3];% no prep trial number assignment is 3
...
%     sma = SetGlobalTimer(sma, 1, S.GUI.RespDur); 
    ...
    if TrialTypes(currentTrial) ==3 % no ready trial
         %if no lick for gatedur  go to GoCue
        sma = AddState(sma, 'Name', 'PreCueGate', ...
            'Timer', S.GUI.PreReadyGate,...
            'StateChangeConditions', {'Tup', 'GoCue',Lick, 'GotoPreCueGate'},...
            'OutputActions', {});
        ...
        sma = AddState(sma, 'Name', 'ReadyCue', ...
            ...
            'StateChangeConditions', {'Tup', 'ReadyCueTup' , Lick,'PreMature_Puni'},...% % add in punishment
    ...
    if TrialTypes(currentTrial) ~=2 % 1 or 3, go trial
    ...
    sma = AddState(sma, 'Name', 'Resp', ...
        ...
        % give water, and goto next trial
    elseif TrialTypes(currentTrial) ==2 % NoGo
        sma = AddState(sma, 'Name', 'GoCue', ...
        'Timer', S.GUI.GoCueDur,...
        'StateChangeConditions', {'Tup', 'CueTup', Lick, 'Resp'},...% 
        'OutputActions', {'Serial1Code', S.GUI.NoGoTone}); % turn cue tone
     %no go punishment 
      sma = AddState(sma, 'Name', 'Resp', ...
        'Timer', 0.0005,...
        'StateChangeConditions', {'Tup', 'NoGoPunish'},...
        'OutputActions', {Port, 255, 'Serial1Code', 255});
     sma = AddState(sma, 'Name', 'NoGoPunish', ...
         'Timer', 0.0005,...
    ...
    TrialTypes = SelectNextTrial(TrialTypes,BpodSystem.Data, S.GUI.Prob_NoGo);
end
%% 
...
% get outcome by trial type
idx_Go = find(Data.TrialTypes==1);
idx_NoGo = find(Data.TrialTypes==2);
idx_Go_noprep = find(Data.TrialTypes==3);

idx_resp = find(Outcomes == 1);
idx_miss = find(Outcomes == 0);
idx_preM = find(Outcomes == -1);

disp(['total trial: ',num2str(length(Outcomes)),...
    '; preM: ', num2str(length(idx_preM))]);
disp(['Go: ', num2str(length(union(idx_Go,idx_Go_noprep))), ...
    '; Collected: ', num2str(length(intersect(idx_Go, idx_resp))),...
    '; noprep Collected: ', num2str(length(intersect(idx_Go_noprep, idx_resp))),...
    '; missed: ', num2str(length(intersect(idx_Go, idx_miss)))]);
disp(['NoGo: ', num2str(length(idx_NoGo)), ...
    '; CR: ', num2str(length(intersect(idx_NoGo, idx_miss))),...
    '; FA: ', num2str(length(intersect(idx_NoGo, idx_resp)))]);

TrialTypeOutcomePlot(BpodSystem.GUIHandles.OutcomePlot,'update',Data.nTrials+1,TrialTypes,Outcomes)
%% 
function [TrialTypes]= SelectNextTrial(TrialTypes, Data, Prob_NoGo)
...
elseif  x < 3 % first 3 trials % 3 trials in a row change it
    ...
    if length(C)==1 % same type for 3 consecutive trials = all three are the same => change
        ...
        %10  no ready trials
        if TrialTypes(x+1) == 1 && ceil(rand(1)*10) == 10;
            TrialTypes((x+1)) = 3;        
        end    
