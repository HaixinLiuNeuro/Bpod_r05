...
% ITI 10-15
% 2017-6-23 modified from ladder
% 2017-8-6 modified from ladder_catch
% 8 sec ladder on following 1s tone, ITI 6-10 (mean 8)
% opto trials: 
%     opto all    15%  = 2 - opto on for the entire cue and ladder period
%     opto ladder 15%  = 3 - opto on only for the ladder period 
%     normal trial    =1
%%
disp('TrialType: 1- normal 2-opto all 3- optoladder');
global BpodSystem
...
for ii = 1:(MaxTrials/20) % 20 trials as a block 3:3:14 pseudo random
% temp = randsample(20,20);
temp = randperm(20);
% temp = rand(1,10)

temp(temp<4) = 2;
temp(temp>3 & temp < 7) = 3;
temp(temp>6) = 1;
    ...
    switch TrialTypes(currentTrial)
        ...
        case 2% opto entire cue + ladder
            ...
            sma = AddState(sma, 'Name', 'GoCue', ...
                'Timer', GoToneDur,...
                'StateChangeConditions', {'Tup', 'LadderOn'},...
                'OutputActions', {'Serial1Code', GoTone, Opto, 255}); %also turn on opto LED
            
            sma = AddState(sma, 'Name', 'LadderOn', ...
                'Timer', S.GUI.Ladder_Dur,...
                'StateChangeConditions', {'Tup', 'exit', STOP, 'LadderOFF'},...
                'OutputActions', {Opto, 255, LadderMotor,255*S.GUI.Ladder_Spd/100, 'Serial1Code', 255}); % also stop gocue
