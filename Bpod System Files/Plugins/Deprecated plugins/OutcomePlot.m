...
switch Action
    case 'init'
        ...
        %add conditions to display taste selections [HL 2016-2-25]
        if max(SideList)<=2 % only 2 choices use conventional left right side
            set(AxesHandle,'TickDir', 'out','YLim', [-1, 2], 'YTick', [0 1],'YTickLabel', {'Right','Left'}, 'FontSize', 16);
        else
            set(AxesHandle,'TickDir', 'out','YLim', [min(SideList)-1, max(SideList)+1], 'YTick', [min(SideList):max(SideList)],'YTickLabel', num2cell([min(SideList):max(SideList)]), 'FontSize', 16);
        end
